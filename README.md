...
它不会只截取字符串中遇到的第一个数字。无法完整解释的非缺失值会变成 `NA`，同时产生警告并记录失败原因，避免脏数据被悄悄转换成错误结果。
...
`parse_cn_quantity()` 保留原文中的限定含义，而不是简单删除“约”“超过”等文字。
...
`parse_cn_range()` 返回上下界以及边界是否包含在区间内，不会擅自用区间平均值替代原始信息。
...
`3-5万` 这类共享单位会自动应用到两个端点。上下界颠倒或端点无法解析时会报告问题。
...
设置 `strict = TRUE` 可以让任意解析失败立即变成错误：
